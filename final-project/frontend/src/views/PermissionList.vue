<template>
  <div class="page">
    <header class="topbar">
      ...
      <div class="actions">
        <el-button type="primary" @click="openDialog()" v-perm="'sys:permission:create'">新增权限</el-button>
      </div>
    </header>
    ...
    <section class="content">
      ...
      >
        ...
        <el-table-column label="操作" width="240" fixed="right">
          <template #default="{ row }">
            <el-button size="small" @click="openDialog(row)" v-perm="'sys:permission:update'">编辑</el-button>
            <el-button size="small" type="warning" @click="handleToggle(row)" v-perm="'sys:permission:toggle'">
              {{ row.status === 1 ? '禁用' : '启用' }}
            </el-button>
            <el-button size="small" type="danger" @click="handleDelete(row)" v-perm="'sys:permission:delete'">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </section>
  ...
  formRef.value.validate(async (valid) => {
    ...
    try {
      ...
      if (isEdit.value) {
        ...
        res = await updatePermission(form.id, {
      ...
      // 检查是否有权限变更通知（仅更新时可能有）
      if (isEdit.value && res.data?.notification) {
