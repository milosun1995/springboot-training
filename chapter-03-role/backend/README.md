...
实现角色管理接口：分页/条件查询、创建、更新、删除、启用/禁用；登录沿用第一章。
...
- 分页查询
```bash
curl "http://localhost:8080/api/roles?page=0&size=10&keyword=ops&status=1" \
...
curl -X POST http://localhost:8080/api/roles \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{"code":"DATA","name":"数据","description":"数据角色","status":1}'
```
...
## API 摘要
- POST `/api/auth/login`
- GET `/api/roles` （page/size/keyword/status）
- POST `/api/roles`
- PUT `/api/roles/{id}`
- DELETE `/api/roles/{id}`
