...
实现菜单树的管理：查询、创建、编辑、删除、状态切换；登录沿用第一章。
...
- 查询菜单树
```bash
curl "http://localhost:8080/api/menus/tree" \
...
curl -X POST http://localhost:8080/api/menus \
  -H "Authorization: Bearer <token)" \
  -H "Content-Type: application/json" \
  -d '{"name":"报表","code":"MENU_REPORT","path":"/report","component":"ReportPage","icon":"DataBoard","status":1}'
```

- 状态切换
```bash
curl -X POST http://localhost:8080/api/menus/1/toggle \
  -H "Authorization: Bearer <token>"
```
...
## API 摘要
- POST `/api/auth/login`
- GET `/api/menus/tree` （支持 keyword/status 过滤）
- POST `/api/menus`
- PUT `/api/menus/{id}`
- DELETE `/api/menus/{id}`
- POST `/api/menus/{id}/toggle`
